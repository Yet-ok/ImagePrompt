...
const WORKFLOW_ID =
  process.env.NEXT_PUBLIC_COZE_WORKFLOW_ID || "7553549738953572406";
...
export async function uploadImageToCoze(
  ...
  try {
    ...
    console.log(
      "使用Token:",
      PERSONAL_TOKEN ? `${PERSONAL_TOKEN.substring(0, 10)}...` : "Not set",
    );
 ...
 * @returns 上传结果
 */
export async function uploadImageUrlToCoze(
  imageUrl: string,
): Promise<CozeUploadResponse> {
  try {
    // 首先获取图片数据
...
export async function runCozeWorkflow(
  ...
  try {
    ...
    if (result.data?.output) {
      ...
      console.log(
        `[${requestId}] 提示词哈希:`,
        await generateSimpleHash(result.data.output),
      );
    }
